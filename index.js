...
function combineAndReverseArrays(arr1, arr2) {
  ...
  if (Array.isArray(arr1, arr2)) {
    let combineAndReverseArrays = arr1.concat(arr2);
    return combineAndReverseArrays.reverse();
  }
  else {
    return [];
  }
...
function customIncludes(arr, element) {
  ...
  const count = arr.filter(item => item === element).length;
    console.log(`Елемент ${element} зустрічається ${count} разів`);
    return count;
  } else {
    return 0;
  }
